...
// The SVG class constructs and returns a logo in svg format
class SVG {
    constructor() {
    ...
    // render() method is called from the "shapes.js" module to return an svg shape element
    setShape(shape) {
        if (shape) {
            this.svgShape = shape.render()
        }
    }
    // svgText and svgShape are passed into a template svg element to generate the final logo element
    render() {
        console.log(`<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg">${this.svgShape}${this.svgText}</svg>`)
